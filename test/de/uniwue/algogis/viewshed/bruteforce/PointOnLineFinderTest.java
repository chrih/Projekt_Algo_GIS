...
public class PointOnLineFinderTest {
	...
	public void testFindPointsOnLine() {
		Point p1 = new Point(1*xFactor, 2*yFactor, 0);
		Point p2 = new Point(3*xFactor, 7*yFactor, 0);
	...
	public void testFindPointsOnLineStraightLeft() {
		Point p1 = new Point(2*xFactor, 3*yFactor, 0);
		Point p2 = new Point(5*xFactor, 3*yFactor, 0);
	...
	public void testFindPointsOnLineStraightUp() {
		Point p1 = new Point(3*xFactor, 2*yFactor, 0);
		Point p2 = new Point(3*xFactor, 5*yFactor, 0);
	...
	private void testFindPointsOnLineHelper(Point p1, Point p2,  int[][] expected) {
		PointOnLineFinder polf = new PointOnLineFinder();
		Collection<Point> result = polf.findPointsOnLine(p1, p2);
		
		assertEquals(expected.length, result.size());
		
		Stream.of(expected).forEach(c -> {
			Point p = new Point(c[0]*xFactor, c[1]*yFactor, 0);
